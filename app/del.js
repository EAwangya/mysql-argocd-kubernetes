...
const mysql = require('mysql2'); // Note: The previous version used mysql2/promise. This one uses mysql2 directly.
...
require('dotenv').config(); // This will load .env files if present, but Docker Compose env vars take precedence.
...
const db = mysql.createConnection({
  host: process.env.MYSQL_HOST || 'mysql_db', // Default to 'mysql_db' for Docker Compose service name
...
app.get('/', (req, res) => {
  db.query('SELECT * FROM users', (err, results) => {
    ...
    db.query('SELECT NOW() as now', (err2, timeResult) => {
      ...
      const hostname = os.hostname(); // This will be the container ID/name in Docker

      let html = `<h3>Users from database: <code>${process.env.MYSQL_DATABASE || 'myapp_db'}</code></h3>`;
      html += `<p>🖥️ This application is running in a Docker container on host: <code>${hostname}</code></p>`;
